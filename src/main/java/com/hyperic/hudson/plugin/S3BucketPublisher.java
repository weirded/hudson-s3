      ...
      for (Upload upload : uploads) {
        ...
        while (!success && currentAttempt <= retries) {
          try {
            ...
            LOGGER.log(Level.WARNING, String.format("Exception uploading file, trying again (retry %d): %s",
              currentAttempt, upload.getDescription()), e);
        ...
        if (!success) {
          log(listener.getLogger(), String.format("All retries for upload failed: %s",
